...
```
/idea-discovery → implement → /baseline-alignment → /parallel-experiment-engine → /auto-review-loop → /paper-writing (optional)
├── Workflow 1 ──┤              ├─ Gate ─┤           ├──────────── Workflow 2 ──────────────┤ ├── Workflow 3 ──┤
```

It orchestrates up to three major workflows plus the implementation bridge and baseline alignment gate between them. Workflow 3 (paper writing) is optional and controlled by `AUTO_WRITE`.
...
dagwrite({ nodes: [
  ...
  { id: "implement",          content: "Implement code via /experiment-bridge",        status: "pending", deps: ["experiment-plan"] },
  { id: "baseline-alignment", content: "Verify results via /baseline-alignment",       status: "pending", deps: ["implement"] },
  { id: "deploy",             content: "Deploy experiments via /parallel-experiment-engine", status: "pending", deps: ["baseline-alignment"] },
  { id: "review-loop",        content: "Auto review loop via /auto-review-loop",       status: "pending", deps: ["deploy"] },
  { id: "paper",              content: "Write paper via /paper-writing",                status: "pending", deps: ["review-loop"] }
]})
```

The `baseline-alignment` gate sits between implementation and deployment — it blocks full deployment if results are catastrophically wrong. The `deploy` node uses `/parallel-experiment-engine` for multi-backend parallel scheduling.
...
2. **Implement the full experiment**:
   ...
   - **Implement baseline reproduction first** — before the method, implement and verify the strongest baseline can reproduce its published numbers (within 15%). This catches data pipeline and evaluation bugs early.
...
### Stage 2.5: Baseline Alignment Gate

Before deploying the full experiment suite, verify that the implementation produces plausible results:
...
This gate:
- Runs the baseline experiment and checks it against published numbers
- Runs a small-scale method experiment and checks it's not catastrophically below baseline
- Auto-debugs common issues (wrong data split, wrong metric, detached gradients) if results are off
- **Blocks full deployment if the gate fails** — no point spending GPU hours on code with bugs

If the gate is **BLOCKED**, return to Stage 2 to fix the implementation. Do not proceed to Stage 3.

### Stage 3: Deploy Experiments (Workflow 2 — Part 1)

Deploy the full-scale experiments using parallel scheduling:
...
- Orchestrates wave transitions (baseline → method → ablation)
...
- **Baseline alignment is mandatory before full deployment.** Never skip the baseline alignment gate (Stage 2.5) for new implementations. A method at 10% on a 60% baseline is a bug, not a weak method.
...
| 2.5. Baseline Alignment | 15-45 min | Yes (auto-debug up to 3 rounds) |
| 3. Deploy | 5 min + experiment time | Yes ✅ (parallel across backends) |
